			...
			<table class="table table-bordered table-striped">
				...
				// paging
					...
					if (isset($_GET['term'])) {
						$sugest = $_GET['term'];
						if ($sugest != null ) {
							$query = mysql_query("select transaksi.idtransaksi, barang.nama, barang.jenis, barang.hrgjual, transaksi.jumlah, transaksi.tgl_transaksi from barang join transaksi on barang.idbarang=transaksi.idbarang where barang.nama like '%$sugest' order by transaksi.idbarang asc limit $mulai, $halaman");
							$sql = mysql_query("select transaksi.idtransaksi, barang.nama, barang.jenis, barang.hrgjual, transaksi.jumlah, transaksi.tgl_transaksi from barang join transaksi on barang.idbarang=transaksi.idbarang where barang.nama like '%$sugest%'");
						}
					}
					else{
						$query = mysql_query("select transaksi.idtransaksi, barang.nama, barang.jenis, barang.hrgjual, transaksi.jumlah, transaksi.tgl_transaksi from barang join transaksi on barang.idbarang=transaksi.idbarang limit $mulai, $halaman");
						$sql= mysql_query("select transaksi.idtransaksi, barang.nama, barang.jenis, barang.hrgjual, transaksi.jumlah, transaksi.tgl_transaksi from barang join transaksi on barang.idbarang=transaksi.idbarang");
					}
					...
					if ($total == 0){
						...
						<td><?php echo number_format($output['jumlah']) ?></td>
			...
			<div class="modal-body">
				<form action="process/tmbh_trans.php" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<?php 
							$idbarang =$q['idbarang'];
						$query2=mysql_query("select stok from barang where idbarang='$idbarang'");
